...
{
    //1.看哪些可以走 2.试着走一下 3.评估结果 4.去最好的结果作为参考
...
{
  //  enum TYPE{CHE, MA, PAO, BING, JIANG, SHI, XIANG};//给每一个棋子进行打分
    ...
    return scoreBlack - scoreRed;//黑棋是机器一方
}
//3.评估结果，最小分数
    ...
    for(int i=min;i<max; i++)//遍历
    {
        ...
        for(int row = 0; row<=9; ++row)
        {
            for(int col=0; col<=8; ++col)//所有行和列的棋子
            {
                ...
                {
                    saveStep(i, killid, row, col, steps); //能够则储存起移动的选择
                }
            }
        }
    }
